...
const ALGO_COLORS = ['#ff6b6b', '#4ecdc4', '#45b7d1', '#f9ca24', '#a29bfe', '#fd79a8'];
...
function BarChart({ data, labels, colors, title }: {
    data: number[];
    labels: string[];
    colors: string[];
    title: string;
    ...
    return (
        <div style={{ marginTop: 12 }}>
            ...
            {labels.map((label, i) => (
                <div key={label} style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 90, fontSize: 11, color: '#aaa', textAlign: 'right', flexShrink: 0 }}>
                        {label}
                    </div>
                    <div style={{
                        flex: 1,
                    ...
                    <div style={{ width: 32, fontSize: 11, color: '#fff', textAlign: 'right' }}>
                        {data[i]}
                    </div>
                </div>
            ))}
...
export default function PopupApp() {
    ...
    const loadStats = useCallback(async () => {
        try {
            ...
            const resp = await chrome.tabs.sendMessage(tabId, { type: 'JUDOL_GET_STATS' });
            if (resp?.stats) {
    ...
    useEffect(() => {
        loadStats();
        chrome.storage.local.get(['judolBlurEnabled', 'judolOcrEnabled', 'judolUseAC', 'judolUseRK'], (result) => {
    ...
    const toggleBlur = async () => {
        ...
        try {
            const tabId = await getActiveTabId();
            if (tabId) {
                await chrome.tabs.sendMessage(tabId, { type: 'JUDOL_TOGGLE_BLUR', enabled: next });
            }
        } catch { }
    };
    ...
    const toggleOcr = async () => {
        ...
        try {
            const tabId = await getActiveTabId();
            if (tabId) {
                await chrome.tabs.sendMessage(tabId, { type: 'JUDOL_TOGGLE_OCR', enabled: next });
            }
        } catch { }
    };
    ...
    const algoNames: AlgorithmType[] = ['kmp', 'boyer-moore', 'regex', 'fuzzy', 'aho-corasick', 'rabin-karp'];
        ...
        <div style={styles.container}>
            ...
            {error && (
                <div style={styles.error}>
                    {error}
                    <button onClick={handleScan} style={styles.retryBtn}>Retry</button>
                </div>
            )}
            ...
            {stats && (
                <div style={styles.statsContainer}>
                    <div style={styles.totalBadge}>
                        {stats.totalMatches} keyword{stats.totalMatches !== 1 ? 's' : ''} found
                    </div>
                    ...
                    <div style={styles.section}>
                        ...
                        <table style={styles.table}>
                            ...
                            <tbody>
                                {algoData.filter(d => d.matches > 0).map(d => (
                                    <tr key={d.name}>
...
const styles: Record<string, React.CSSProperties> = {
    ...
    logo: { fontSize: 24 },
    title: { fontSize: 18, fontWeight: 700, color: '#fff' } as React.CSSProperties,
